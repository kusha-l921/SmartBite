...
class MenuWindow(QMainWindow):
    ...
    def update_items(self):
        ...
        for item, price in items:
            self.items_combo.addItem(f"{item} - ${price:.3f}", (item, price))
    ...
    def add_item(self):
        ...
        if item_data:
            ...
            self.order_list.addItem(f"{item} - ${price:.3f}")
            self.total_label.setText(f"Total: ${self.total_price:.3f}")
    ...
    def complete_order(self):
        ...
        save_to_csv(self.order_history_file, [self.user, order_summary, f"${self.total_price:.3f}"])
        QMessageBox.information(self, "Order Complete", f"Your order has been placed.\nTotal: ${self.total_price:.3f}")
